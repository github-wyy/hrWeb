<template>
  <div>
    <div class="title" style="text-align:left">
      <font-awesome-icon icon="th-large" />&nbsp;创立部门
    </div>
    ...
    <el-form ref="form" :model="form" label-width="120px" :rules="rules"
      ...
      <el-form-item>
        <el-col :sm="16">
          <el-button type="primary" @click="submitForm">创立</el-button>
      ...
      this.$refs['form'].validate((valid) => {
        if (valid) {
          ...
          this.$nextTick(() => { // 以服务的方式调用的 Loading 需要异步关闭
            setTimeout(() => {
              this.$back.dept.add(this, this.form).then(res => { // 加载新闻数据信息
                console.log(res)
                ...
                if (res.result.status === 1000) {
                  this.$notify({
                    title: '部门创建成功',
                    message: h('i', {style: 'color: teal'}, '新部门已经成功创建')
                  })
                } else {
                  this.$notify({
                    title: '部门创建失败',
